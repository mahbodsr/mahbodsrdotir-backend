...
app.get("/api/phonecode/:phonecode", async (req) => {
    event.emit("phonecode", req.params.phonecode);
});
(async () => {
    const videos = new Map();
    const client = new TelegramClient(new StoreSession("mahbodsr_second"), +process.env.API_ID, process.env.API_HASH, {});
    app.listen(PORT, () => {
    ...
    client.session.save();
    app.get("/api/stream/:chatId/:messageId", authenticateJWT, async (req, res) => {
